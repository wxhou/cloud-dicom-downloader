...
- 搜索现有工作：`openspec-chinese spec list --long`、`openspec-chinese list`（仅在全文搜索时使用 `rg`）
- 决定范围：新功能 vs 修改现有功能
...
- 验证：`openspec-chinese validate [change-id] --strict`并修复问题
- 请求批准：在提案被批准前不要开始实施
...
**工作流程**
1. 查看`openspec/project.md`、`openspec-chinese list`和`openspec-chinese list --specs`了解当前上下文。
...
4. 运行`openspec-chinese validate <id> --strict`并在共享提案前解决任何问题。
...
- 如果功能发生变化，更新`specs/`
- 对于仅工具变更，使用`openspec-chinese archive <change-id> --skip-specs --yes`（始终明确传递变更ID）
- 运行`openspec-chinese validate --strict`确认归档的变更通过检查
...
- [ ] 运行`openspec-chinese list`查看活跃变更
- [ ] 运行`openspec-chinese list --specs`查看现有功能

**创建规范之前：**
- 始终检查功能是否已存在
- 优先修改现有规范而非创建重复项
- 使用`openspec-chinese show [spec]`查看当前状态
...
# 调试
openspec-chinese show [change] --json --deltas-only
...
- 调试：`openspec-chinese show [change] --json --deltas-only`
...
# 调试增量解析
openspec-chinese show [change] --json | jq '.deltas'

# 检查特定需求
openspec-chinese show [spec] --json -r 1
```
...
### 变更冲突
1. 运行`openspec-chinese list`查看活跃变更
2. 检查重叠规范
3. 与变更所有者协调
4. 考虑组合提案
...
### CLI基本命令
```bash
openspec-chinese list              # 进行中有什么？
openspec-chinese show [item]       # 查看详情
openspec-chinese validate --strict # 是否正确？
openspec-chinese archive <change-id> [--yes|-y]  # 标记完成（自动化添加--yes）
```
